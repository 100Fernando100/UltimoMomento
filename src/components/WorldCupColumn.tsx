  ...
  return (
    <section className="w-full">
      <div className="flex items-center gap-3 mb-4 pb-3" style={{ borderBottom: '2px solid #e65c00' }}>
        ...
        <span className="font-oswald font-700 uppercase tracking-wider text-sm" style={{ color: '#111' }}>MUNDIAL 2026: LA COBERTURA QUE MERECES</span>
        <span className="exclusive-badge blink text-xs">EN VIVO</span>
      </div>

      <div className="mb-5 px-3 py-2 flex items-center gap-3 flex-wrap" style={{ background: '#f0fff0', border: '1px solid #c0e0c0' }}>
        ...
        {['🇺🇸 USA', '🇨🇦 CANADA', '🇲🇽 MEXICO'].map(s => (
          <span key={s} className="font-oswald text-xs" style={{ color: '#555' }}>{s}</span>
        ))}
        <span className="ml-auto font-oswald text-xs blink-slow" style={{ color: '#7acc00' }}>48 EQUIPOS · 104 PARTIDOS · MAXIMA TENSION</span>
      </div>

      <div className="mb-6 p-3" style={{ background: '#f8f8f8', border: '1px solid #e0e0e0' }}>
        ...
        <div className="flex flex-col gap-2">
          {liveScores.map((s, i) => (
            <div key={i} className="flex items-center gap-2 py-2" style={{ borderBottom: '1px solid #e5e5e5' }}>
              <span
                className="font-oswald font-700 flex-1 text-right text-xs uppercase"
                style={{ maxWidth: 130, color: '#111' }}
              >
                {s.team1}
              </span>
              <div className="flex items-center gap-1 flex-shrink-0">
                <span className="font-anton" style={{ fontSize: '1.1rem', lineHeight: 1, color: '#111' }}>{s.score1}</span>
                <span
                  className="font-oswald text-xs px-1.5 py-0.5 font-700"
                  style={{
                    background: s.status === 'EN JUEGO' ? '#cc0000' : s.status === 'FINALIZADO' ? '#555' : '#2a7a00',
                    color: 'white',
                    fontSize: '0.55rem',
                    letterSpacing: '0.1em',
                  }}
                ...
                <span className="font-anton" style={{ fontSize: '1.1rem', lineHeight: 1, color: '#111' }}>{s.score2}</span>
              </div>
              <span className="font-oswald font-700 flex-1 text-left text-xs uppercase" style={{ maxWidth: 130, color: '#111' }}>
                {s.team2}
              </span>
            </div>
          ))}
      ...
      <div className="flex flex-col gap-5">
        {worldCupNews.map((item, i) => (
          <div key={i} className="news-card cursor-pointer group">
            ...
            <div className="pt-3">
              <h3 className="font-anton leading-tight text-base group-hover:text-orange-600 transition-colors mb-2" style={{ color: '#111' }}>
                {item.title}
              </h3>
              <p className="font-oswald text-xs leading-relaxed" style={{ color: '#777' }}>{item.detail}</p>
            </div>
          </div>
        ))}
      </div>

      <div className="mt-5 p-4 flex items-center gap-3" style={{ background: '#fff8f0', border: '1px solid #e65c00' }}>
        <Star size={14} style={{ color: '#e65c00' }} />
        <div>
          <p className="font-oswald font-700 text-xs uppercase tracking-wider" style={{ color: '#111' }}>PRONOSTICO OFICIAL</p>
          <p className="font-oswald text-xs mt-0.5" style={{ color: '#666' }}>
